...
import type { SeoGeneratorType, GenerateSeoContentResponse } from "@/lib/tools/seo-generator/types";

interface UseSeoGeneratorResult {
  topic: string;
...
/** Drives the Meta Title / Meta Description generators. Mirrors every other platform's generator hook exactly. */
export function useSeoGenerator(type: SeoGeneratorType): UseSeoGeneratorResult {
  ...
  const runGeneration = React.useCallback(
    async (currentTopic: string) => {
      const parsed = seoGeneratorFormSchema.safeParse({ topic: currentTopic });
      if (!parsed.success) {
      ...
      try {
        const response = await fetch("/api/tools/seo-generator/generate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ type, topic: parsed.data.topic }),
        });

        const data = (await response.json()) as GenerateSeoContentResponse;
