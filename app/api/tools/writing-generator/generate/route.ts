...
import { WRITING_GENERATOR_CONFIG } from "@/lib/tools/writing-generator/constants";
import type { GenerateWritingContentResponse, WritingGeneratorType } from "@/lib/tools/writing-generator/types";
...
function isValidGeneratorType(value: unknown): value is WritingGeneratorType {
  return typeof value === "string" && value in WRITING_GENERATOR_CONFIG;
}

function buildPrompt(type: WritingGeneratorType, topic: string): string {
  const config = WRITING_GENERATOR_CONFIG[type];
  return `You are an expert writer. A user gives you context; you produce ${OUTPUT_COUNT} distinct, high-quality options for them to choose from.
...
- Work only from the context given — do not invent facts, achievements, or details not mentioned.
...
export async function POST(request: NextRequest): Promise<NextResponse<GenerateWritingContentResponse>> {
  try {
    ...
    const parseResult = writingGeneratorFormSchema.safeParse({ topic: bodyRecord.topic });
    ...
    console.error("[/api/tools/writing-generator/generate] Generation failed:", error);
