...
import { TWITTER_GENERATOR_CONFIG } from "@/lib/tools/twitter-generator/constants";
import type {
...
function isValidGeneratorType(value: unknown): value is TwitterGeneratorType {
  return typeof value === "string" && value in TWITTER_GENERATOR_CONFIG;
}

function buildPrompt(type: TwitterGeneratorType, topic: string): string {
  const config = TWITTER_GENERATOR_CONFIG[type];

  return `You are an expert X (Twitter) content strategist and copywriter. A user gives you a topic; you produce ${OUTPUT_COUNT} distinct, high-quality options for them to choose from.
...
export async function POST(
  request: NextRequest
): Promise<NextResponse<GenerateTwitterContentResponse>> {
  try {
    ...
    const parseResult = twitterGeneratorFormSchema.safeParse({ topic: bodyRecord.topic });
    ...
    console.error("[/api/tools/twitter-generator/generate] Generation failed:", error);
