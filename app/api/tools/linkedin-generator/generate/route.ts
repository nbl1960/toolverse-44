...
import { LINKEDIN_GENERATOR_CONFIG } from "@/lib/tools/linkedin-generator/constants";
import type {
...
function isValidGeneratorType(value: unknown): value is LinkedinGeneratorType {
  return typeof value === "string" && value in LINKEDIN_GENERATOR_CONFIG;
}

function buildPrompt(type: LinkedinGeneratorType, topic: string): string {
  const config = LINKEDIN_GENERATOR_CONFIG[type];

  return `You are an expert LinkedIn profile strategist and professional copywriter. A user gives you context; you produce ${OUTPUT_COUNT} distinct, high-quality options for them to choose from.
...
- Keep the tone professional but human — avoid generic corporate buzzword soup.
...
export async function POST(
  request: NextRequest
): Promise<NextResponse<GenerateLinkedinContentResponse>> {
  try {
    ...
    const parseResult = linkedinGeneratorFormSchema.safeParse({ topic: bodyRecord.topic });
    ...
    console.error("[/api/tools/linkedin-generator/generate] Generation failed:", error);
